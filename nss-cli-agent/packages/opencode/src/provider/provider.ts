  ...
  return {
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://TODO_NSS_DOMAIN/",
            "X-Title": "nss-cli-agent",
            "X-Source": "nss-cli-agent",
          },
        },
      }),
    openrouter: () =>
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://TODO_NSS_DOMAIN/",
            "X-Title": "nss-cli-agent",
            "X-BILLING-INVOKE-ORIGIN": "NSS CLI",
          },
        },
      }),
    vercel: () =>
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "nss-cli-agent",
          },
        },
      }),
    kilo: () =>
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        if (!database["xuanzhi"]) {
          const xuanzhiID = ProviderV2.ID.make("xuanzhi")
          database["xuanzhi"] = {
            id: xuanzhiID,
            name: "玄知",
            env: ["XUANZHI_API_KEY"],
            ...
            models: {
              "xuanzhi-default": {
                id: ProviderV2.ModelID.make("xuanzhi-default"),
                providerID: xuanzhiID,
                name: "玄知",
                api: {
                  id: "xuanzhi-default",
                  npm: "@ai-sdk/openai-compatible",
                  url: "https://api.xuanzhi.ai/v1",
                },
                status: "active",
                capabilities: {
                  temperature: true,
        ...
        const NSS_ALLOWED = new Set(["xuanzhi", "alibaba-cn", "openai", "github-copilot", "anthropic", "google"])
        const enabled = cfg.enabled_providers
          ? new Set([...cfg.enabled_providers].filter((x) => NSS_ALLOWED.has(x)))
          : NSS_ALLOWED
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const alibabaCn = s.providers[ProviderV2.ID.make("alibaba-cn")]
      if (alibabaCn) {
        const [model] = sort(Object.values(alibabaCn.models))
        if (model) return { providerID: alibabaCn.id, modelID: model.id }
      }
