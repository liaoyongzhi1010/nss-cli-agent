...
const ALLOWED_PROVIDERS = new Set(Object.keys(PROVIDER_PRIORITY))
const NSS_FEATURED = new Set(["xuanzhi", "alibaba-cn"])

export function providerOptions(list: { id: string; name: string }[]): ProviderOption[] {
  const augmented = list.some((x) => x.id === "xuanzhi")
    ? list
    : [{ id: "xuanzhi", name: "玄知" }, ...list]
  ...
  return pipe(
    ...
    map((provider) => ({
      ...
      category: NSS_FEATURED.has(provider.id) ? "Popular" : "Providers",
    })),
  )
}
          ...
          async onSelect() {
            if (providerID === "xuanzhi") {
              dialog.replace(() => (
                <box paddingLeft={2} paddingRight={2} gap={1} paddingBottom={1}>
                  ...
                  <text fg={theme.textMuted}>玄知大模型暂不支持，敬请期待</text>
                </box>
              ))
              return
            }
            ...
            if (connected) {
                ...
                dialog.replace(
                  () => (
                    <DialogSelect
                      title={provider.title}
                      options={[
                        { title: "重新配置", value: "reconnect" as const, description: "更新 API Key" },
                        { title: "断开连接", value: "disconnect" as const, description: "移除已保存的凭证" },
                      ]}
              ...
              if (action === "disconnect") {
                ...
                dialog.clear()
                toast.show({ variant: "info", message: `已断开 ${provider.title}` })
                return
              }
            }
      ...
      description={
        {
          opencode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                NSS CLI gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
          ...
          "opencode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                NSS CLI Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://TODO_NSS_DOMAIN/go</span> and enable NSS CLI Go
              </text>
            </box>
          ),
