...
const MANAGED_PLIST_DOMAIN = "ai.nss-cli.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/nss-cli"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "nss-cli")
    default:
      return "/etc/nss-cli"
  }
}
...
export function managedConfigDir() {
  return process.env.NSS_CLI_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
